...
(function () {
  ...
  /*
   * הצבע של חלק נגזר מזהותו (piece.id) ולא מגודלו — כל צורה מקבלת צבע
   * קבוע משלה מתוך פלטה של 8, כך שהמגש נראה כמו קשת בסגנון המשחק המקורי.
   * הזהות קבועה, ולכן הצבע יציב בין רינדורים ובין טעינות (וגם ה-save
   * ששומר את מחרוזת הצבע נשאר תקף).
   */
  const PALETTE = 8;
  const pieceColor = (piece) => {
    const id = String(piece.id || piece.size || '');
    let h = 0;
    for (let i = 0; i < id.length; i++) h = (h * 31 + id.charCodeAt(i)) >>> 0;
    return 'var(--bb-p' + (h % PALETTE + 1) + ')';
  };
